...
class RosLifecycleManager(Plugin):
    ...
    def __init__(self, context):
        ...
        # Initially disable all buttons except Refresh and Shutdown
    ...
    def _refresh_lc_node_list(self):
        ...
        if self.node_list:
            ...
            self._widget.lifecycleNodeList.setCurrentIndex(0)
            self._on_node_selection_changed(0)

            # After refresh, only Configure enabled
            self._widget.pushConfigureObj.setEnabled(True)
            self._widget.pushActivateObj.setEnabled(False)
            self._widget.pushDeactivateObj.setEnabled(False)
            self._widget.pushCleanObj.setEnabled(False)
        else:
            # No nodes found – disable all control buttons
            self._widget.pushConfigureObj.setEnabled(False)
            self._widget.pushActivateObj.setEnabled(False)
            self._widget.pushDeactivateObj.setEnabled(False)
            self._widget.pushCleanObj.setEnabled(False)
    ...
    def _on_node_selection_changed(self, index):
        ...
        if node_name:
            ...
            # Update buttons according to the node state and your logic:
            if state.label == 'unconfigured':
                # Only Configure enabled
                self._widget.pushConfigureObj.setEnabled(True)
                self._widget.pushActivateObj.setEnabled(False)
                self._widget.pushDeactivateObj.setEnabled(False)
                self._widget.pushCleanObj.setEnabled(False)
            elif state.label == 'configuring' or state.label == 'inactive':
                # After configure: Clean Up and Activate enabled
                self._widget.pushConfigureObj.setEnabled(False)
                self._widget.pushActivateObj.setEnabled(True)
                self._widget.pushCleanObj.setEnabled(True)
                self._widget.pushDeactivateObj.setEnabled(False)
            elif state.label == 'activating' or state.label == 'active':
                # After activate: Deactivate enabled only
                self._widget.pushConfigureObj.setEnabled(False)
                self._widget.pushActivateObj.setEnabled(False)
                self._widget.pushDeactivateObj.setEnabled(True)
                self._widget.pushCleanObj.setEnabled(False)
            elif state.label == 'deactivating' or state.label == 'finalized' or state.label == 'shuttingdown':
                # Disable all buttons during transition or shutdown
                self._widget.pushConfigureObj.setEnabled(False)
                self._widget.pushActivateObj.setEnabled(False)
                self._widget.pushDeactivateObj.setEnabled(False)
                self._widget.pushCleanObj.setEnabled(False)
    ...
    def _configure_lc_node(self):
        ...
        if node_name:
            ...
            if result:
                ...
                # Enable Clean Up and Activate after configure success
                self._widget.pushConfigureObj.setEnabled(False)
                self._widget.pushCleanObj.setEnabled(True)
                self._widget.pushActivateObj.setEnabled(True)
                self._widget.pushDeactivateObj.setEnabled(False)
    ...
    def _activate_lc_node(self):
        ...
        if node_name:
            ...
            if result:
                self.draw_state_machine(state.label, 'success')
                # Enable Deactivate after activate success
                self._widget.pushDeactivateObj.setEnabled(True)
                self._widget.pushConfigureObj.setEnabled(False)
                self._widget.pushActivateObj.setEnabled(False)
                self._widget.pushCleanObj.setEnabled(False)
    ...
    def _deactivate_lc_node(self):
        ...
        if node_name:
            ...
            if result:
                self.draw_state_machine(state.label, 'success')
                # After deactivation, disable all buttons until next refresh
                self._widget.pushConfigureObj.setEnabled(False)
                self._widget.pushActivateObj.setEnabled(False)
                self._widget.pushDeactivateObj.setEnabled(False)
                self._widget.pushCleanObj.setEnabled(False)
    ...
    def _cleanup_lc_node(self):
        ...
        if node_name:
            ...
            if result:
                self.draw_state_machine(state.label, 'success')
                # Enable Deactivate after cleanup success
                self._widget.pushDeactivateObj.setEnabled(True)
                self._widget.pushConfigureObj.setEnabled(False)
                self._widget.pushActivateObj.setEnabled(False)
                self._widget.pushCleanObj.setEnabled(False)
